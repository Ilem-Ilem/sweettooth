<div class="p-3 space-y-3">
    ...
    <x-table
        :headers="[
            ['index' => 'purchase_number', 'label' => 'Purchase #'],
            ['index' => 'purchase_date', 'label' => 'Date'],
            ['index' => 'supplier_name', 'label' => 'Supplier'],
            ['index' => 'total_fob_ngn', 'label' => 'Total FOB (NGN)'],
            ['index' => 'landing_cost', 'label' => 'Landing Cost'],
        ...
        @interact('column_purchase_number', $row)
        ...
        @interact('column_purchase_date', $row)
            <span class="text-zinc-900 dark:text-zinc-100">
                {{ $row->purchase_date ? $row->purchase_date->format('Y-m-d') : 'N/A' }}
            </span>
        @endinteract

        @interact('column_supplier_name', $row)
            <div>
                <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $row->supplier_name }}</p>
                @if($row->supplier_contact)
                <p class="text-xs text-zinc-600 dark:text-zinc-400">{{ $row->supplier_contact }}</p>
                @endif
            </div>
        @endinteract

        @interact('column_total_fob_ngn', $row)
            <span class="text-zinc-900 dark:text-zinc-100">
                ₦{{ number_format($row->total_fob_ngn, 2) }}
            </span>
        @endinteract

        @interact('column_landing_cost', $row)
            <span class="font-medium text-zinc-900 dark:text-zinc-100">
                ₦{{ number_format($row->landing_cost, 2) }}
            </span>
        @endinteract
        ...
        @interact('column_payment_status', $row)
            ...
            @endphp
            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $statusColors[$row->payment_status] ?? '' }}">
            ...
            <div class="flex-1 overflow-y-auto px-6 py-4 scrollbar-thin">
                <form wire:submit.prevent="save" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Exchange Rate *</label>
                            ...
                            <table class="w-full">
                                <thead class="bg-zinc-50 dark:bg-zinc-800">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Unit FOB (FC)</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-t border-zinc-200 dark:border-zinc-700">
                                        ...
                                        <td class="px-4 py-2">
                                            <input type="number" step="1" wire:model="purchaseItems.{{ $index }}.quantity"
                                        ...
                                        <td class="px-4 py-2">
                                            <select wire:model="purchaseItems.{{ $index }}.uom" id=""  class="w-24 px-2 py-1 text-sm border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100">
                                        ...
                                        <td class="px-4 py-2">
                                            <input type="number" step="1" wire:model="purchaseItems.{{ $index }}.unit_fob_fc"
                                                class="w-28 px-2 py-1 text-sm border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100">
                                        </td>
                                        <td class="px-4 py-2">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-zinc-200 dark:border-zinc-700">
                    ...
                    <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-2">
                        @switch($auditAction)
                            @case('create_purchase')
                                Please explain the purpose of this purchase order.
                                @break
                            @case('delete_purchase')
                                This purchase will be permanently deleted. This action cannot be undone.
                                @break
                            @default
                                ...
                                $wire
                                    .call(
                                        @js(match($auditAction) {
                                            'create_purchase' => 'submitPurchaseApprovalRequest',
                                            'delete_purchase' => 'submitPurchaseDeletionRequest',
                                            default => 'submitPurchaseApprovalRequest'
                                        }))
