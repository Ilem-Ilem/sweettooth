<div class="p-3 space-y-3">
            ...
            <div>
                ...
                <select wire:model.live="selectedItemId"
                    ...
                    @foreach($items as $item)
                        <option value="{{ $item->id }}" @selected($selectedItemId == $item->id)>{{ $item->name }} ({{ $item->sku }})</option>
                    @endforeach
                </select>
            </div>
            <div>
    ...
    @if($selectedItem)
        <!-- Loading Indicator -->
        <div wire:loading class="fixed top-4 right-4 z-50 bg-blue-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center space-x-2">
                ...
                <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-4 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold mt-1">{{ number_format($stockSummary->current_available, 2) }} {{ $selectedItem->uom }}</p>
                        </div>
                        <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-lg p-4 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold mt-1">{{ number_format($stockSummary->avg_movement, 2) }} {{ $selectedItem->uom }}</p>
                        </div>
                        <div class="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4" wire:key="charts-container-{{ $selectedItemId }}">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <select wire:model.live="trendViewMode" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        @if($trendViewMode === 'chart')
                            <select wire:model.live="trendChartType" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="overflow-x-auto scrollbar-thin">
                        <table class="w-full text-sm">
                            <thead class="bg-zinc-50 dark:bg-zinc-900/50 sticky top-0">
                                <tr>
                                    <th class="px-2 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Date</th>
                                    <th class="px-2 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Before</th>
                                    <th class="px-2 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">After</th>
                                    <th class="px-2 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Type</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @forelse($stockMovementTrend as $movement)
                                    <tr>
                                        <td class="px-2 py-2 text-zinc-900 dark:text-zinc-100">{{ $movement->movement_date->format('Y-m-d H:i') }}</td>
                                        <td class="px-2 py-2 text-right text-zinc-600 dark:text-zinc-400">{{ number_format($movement->quantity_before, 2) }}</td>
                                        <td class="px-2 py-2 text-right text-zinc-900 dark:text-zinc-100 font-semibold">{{ number_format($movement->quantity_after, 2) }}</td>
                                        <td class="px-2 py-2">
                                            <span class="px-2 py-0.5 text-xs rounded-full {{ $movement->type === 'in' || $movement->type === 'return' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-2 py-6 text-center text-zinc-500">No data available</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <select wire:model.live="typesViewMode" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        @if($typesViewMode === 'chart')
                            <select wire:model.live="typesChartType" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="overflow-x-auto scrollbar-thin">
                        <table class="w-full text-sm">
                            <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                                <tr>
                                    <th class="px-2 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Type</th>
                                    <th class="px-2 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Quantity</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @forelse($movementsByType as $movement)
                                    <tr>
                                        <td class="px-2 py-2 text-zinc-900 dark:text-zinc-100">{{ ucfirst($movement->type) }}</td>
                                        <td class="px-2 py-2 text-right text-zinc-900 dark:text-zinc-100 font-semibold">{{ number_format($movement->total, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-2 py-6 text-center text-zinc-500">No data available</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <select wire:model.live="deptViewMode" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        @if($deptViewMode === 'chart')
                            <select wire:model.live="deptChartType" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                ...
                @else
                    <div class="overflow-x-auto scrollbar-thin">
                        <table class="w-full text-sm">
                            <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                                <tr>
                                    <th class="px-2 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Department</th>
                                    <th class="px-2 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Quantity</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @forelse($departmentUsage as $dept)
                                    <tr>
                                        <td class="px-2 py-2 text-zinc-900 dark:text-zinc-100">{{ $dept->department_name }}</td>
                                        <td class="px-2 py-2 text-right text-zinc-900 dark:text-zinc-100 font-semibold">{{ number_format($dept->total_quantity, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-2 py-6 text-center text-zinc-500">No data available</td>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <select wire:model.live="freqViewMode" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        @if($freqViewMode === 'chart')
                            <select wire:model.live="freqChartType" class="px-2 py-1 text-xs border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500">
                ...
                @if($freqViewMode === 'chart')
                    <div id="freqChart" style="height: 300px;"></div>
                @else
                    <div class="overflow-x-auto scrollbar-thin">
                        <table class="w-full text-sm">
                            <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                                <tr>
                                    <th class="px-2 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Date</th>
                                    <th class="px-2 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Count</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                                @forelse($movementFrequency as $freq)
                                    <tr>
                                        <td class="px-2 py-2 text-zinc-900 dark:text-zinc-100">{{ $freq->date }}</td>
                                        <td class="px-2 py-2 text-right text-zinc-900 dark:text-zinc-100 font-semibold">{{ $freq->count }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="2" class="px-2 py-6 text-center text-zinc-500">No data available</td>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold text-green-600 dark:text-green-400 mt-1">{{ number_format($stockSummary->total_in, 2) }}</p>
                        </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold text-red-600 dark:text-red-400 mt-1">{{ number_format($stockSummary->total_out, 2) }}</p>
                        </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ number_format($stockSummary->total_adjustments, 2) }}</p>
                        </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">{{ number_format($stockSummary->total_damaged, 2) }}</p>
                        </div>
    ...
    @else
        <div class="bg-zinc-100 dark:bg-zinc-800 rounded-lg p-12 text-center">
            ...
            <p class="text-sm text-zinc-500 dark:text-zinc-500 mt-1">Please select an item from the dropdown above to view analytics</p>
        </div>
    @endif
                    ...
                    <table class="w-full">
                        <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Movements</th>
                                <th class="px-4 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Total Qty</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($topMovingItems as $item)
                                <tr>
                                    <td class="px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100">{{ $item->name }}</td>
                                    <td class="px-4 py-2 text-sm text-right text-zinc-900 dark:text-zinc-100">{{ $item->movement_count }}</td>
                                    <td class="px-4 py-2 text-sm text-right text-zinc-900 dark:text-zinc-100">{{ number_format($item->total_quantity, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-6 text-center text-zinc-500">No data available</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-zinc-700 dark:text-zinc-300">Item</th>
                                <th class="px-4 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Available</th>
                                <th class="px-4 py-2 text-right text-xs font-medium text-zinc-700 dark:text-zinc-300">Reorder</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            @forelse($lowStockItems as $item)
                                <tr>
                                    <td class="px-4 py-2 text-sm text-zinc-900 dark:text-zinc-100">{{ $item->name }}</td>
                                    <td class="px-4 py-2 text-sm text-right text-red-600 dark:text-red-400 font-semibold">{{ number_format($item->quantity_available, 2) }}</td>
                                    <td class="px-4 py-2 text-sm text-right text-zinc-600 dark:text-zinc-400">{{ number_format($item->reorder_level, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-6 text-center text-green-600 dark:text-green-400">All items are well stocked!</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderTrendChart() {
                ...
                if (viewMode !== 'chart' || !data || data.length === 0) return;
                ...
                const options = {
                    ...
                    fill: {
                        type: chartType === 'line' ? 'solid' : 'solid',
                    },
                    dataLabels: {
                        enabled: false,
                    },
            ...
            function renderTypesChart() {
                ...
                if (viewMode !== 'chart' || !data || Object.keys(data).length === 0) return;
                ...
                const options = {
                    ...
                    series: chartType === 'bar' ? [{ name: 'Quantity', data: values }] : values,
                    labels: labels,
            ...
            function renderDeptChart() {
                ...
                const options = {
                    ...
                    series: chartType === 'bar' ? [{ name: 'Quantity Dispatched', data: values }] : values,
                    labels: labels,
            ...
            // Re-render on Livewire updates and custom events
