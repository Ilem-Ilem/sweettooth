...
class InventoryStockVarianceDefinition implements ReportDefinition
{
    ...
    public function query(array $context): array
    {
        $stockTakes = StockTake::query()
            ...
            ->whereBetween('stock_take_date', [$context['period_from'], $context['period_to']])
        ...
        foreach ($stockTakes as $stockTake) {
            foreach ($stockTake->stockTakeItems as $stockTakeItem) {
                ...
                if ($variance > 0) {
                    $positiveVariances[] = $record;
                } elseif ($variance < 0) {
                    $negativeVariances[] = $record;
                }

                if (abs($variancePercentage) > 5 || abs($record['variance_value']) > 1000) {
                    $significantVariances[] = $record;
                }
            }
        }
    ...
    {
        $overview = $data['variance_overview'] ?? [];

        return [
            'total_items' => $overview['total_items_checked'] ?? 0,
            'accuracy_rate' => $overview['accuracy_rate'] ?? 0,
            'total_variance_value' => $overview['total_variance_value'] ?? 0,
            'critical_variances' => $overview['critical_variances'] ?? 0,
            'items_with_variance' => $overview['items_with_variance'] ?? 0,
        ];
    }
    ...
    public function charts(array $data, array $context): array
    {
        ...
        return [
            'severity_breakdown_chart' => [
                ...
                'data' => [
                    $overview['critical_variances'] ?? 0,
                    $overview['high_variances'] ?? 0,
                    $overview['medium_variances'] ?? 0,
                    $overview['low_variances'] ?? 0,
                ],
            ...
            'variance_value_chart' => [
                ...
                'datasets' => [
                    [
                        ...
                        'data' => [
                            $overview['positive_variance_value'] ?? 0,
                            $overview['negative_variance_value'] ?? 0,
                        ],
    ...
    public function tables(array $data, array $summary, array $context): array
    {
        return [
            'all_variances' => [
                'headers' => ['Date', 'Item', 'SKU', 'Category', 'System', 'Actual', 'Variance', 'Value', 'Severity'],
            ...
            'significant_variances' => [
                'headers' => ['Item', 'Variance %', 'Value', 'Severity'],
                'rows' => array_map(function ($row) {
                    return [
                        $row['item_name'] ?? '-',
    ...
    public function narrative(array $data, array $summary, array $context): array
    {
        ...
        $accuracy = $summary['accuracy_rate'] ?? 0;
        if ($accuracy >= 98) {
            $highlights[] = "Inventory accuracy is high at {$accuracy}%.";
        } elseif ($accuracy < 90) {
            $concerns[] = "Inventory accuracy is low at {$accuracy}%.";
        }

        if (($summary['critical_variances'] ?? 0) > 0) {
            $concerns[] = 'Critical variances detected in stock counts.';
        }

        return [
            'overview' => "Stock variance analysis for {$context['period_from']} to {$context['period_to']}.",
            'highlights' => $highlights,
            'concerns' => $concerns,
            'recommendations' => [
                'Investigate high‑severity variances and adjust stock control procedures.',
            ],
        ];
    }
