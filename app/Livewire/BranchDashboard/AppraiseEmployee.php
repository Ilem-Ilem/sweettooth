...
class AppraiseEmployee extends Component
{
    ...
    public function mount($employee)
    {
        ...
        $this->branch = request()->query('branch') ?? auth()->user()->branch_id ?? auth()->user()->last_accessed_branch_id;

        // Verify the employee belongs to this branch
        if ($this->employee->branch_id !== $this->branch) {
            abort(403, 'Unauthorized access to this employee.');
        }

        // Set default cycle and template
        $defaultCycle = AppraisalCycle::active()->forDepartment($this->employee->department_id)->first();
