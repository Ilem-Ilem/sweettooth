...
class StockLevelAnalytics extends Component
{
    ...
    // Get stock movements over time to track stock level changes
        $movementsQuery = StockMovement::with('stock.item');

    if ($this->selectedBranch) {
    $movementsQuery->whereHas('stock', function ($query) {
    ...
    $dailyLevelsQuery = Stock::query();

    if ($this->selectedBranch) {
                $dailyLevelsQuery->where('branch_id', $this->selectedBranch);
            }

            $dailyLevels = $dailyLevelsQuery
    ...
    public function render()
    {
        ...
        return view('livewire.super-admin.analytics.stock-level-analytics', [
            'stocks' => $stocks,
